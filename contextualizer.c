...
{
    contextualizer = (Contextualizer*)malloc(sizeof(Contextualizer));
    contextualizer->stackIndex = 0;
    contextualizer->generatedByte = 0;
    contextualizer->generatedByteAux = 0;
    contextualizer->contextualizerError = malloc(1024 * sizeof(char));
...
void executeAction(Contextualizer* contextualizer, int action, Token* token)
{
    ...
    case PROGRAM_END:
        ...
        {
            char* label = contextualizer->labelsBuffer.list[i].key;

            if(label != NULL )
            {
                if(get(&contextualizer->labels, label) != -1)
                {
                    int command = get(&contextualizer->labels, label) - 3;
                    List* list = getList(&contextualizer->labelsBuffer, label);
                ...
                {
                    contextualizer->contextualizerError = "Label does not exist";
                }
            }
        }
    ...
    case LABEL_RECORD:
        ...
        {
            contextualizer->contextualizerError = "Duplicated label";
        }
        break;
